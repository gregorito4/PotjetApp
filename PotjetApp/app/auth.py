...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            if (not email or (not utils.isEmailValid(email))):
                error = 'Dirección de correo no válido'
                flash(error)
                return render_template('auth/forgot.html')
            ...
            if user is not None:
                ...
                content = 'Hola, para cambiar tu contraseña, por favor haz clic en este enlace ' + flask.url_for('auth.change', _external=True) + '?auth=' + number
                
                send_email(credentials, receiver=email, subject='Nueva contraseña', message=content)
                
                flash('Por favor verifica tu correo electrónico registrado')
            else:
